        ...
        <div className="px-4 py-2 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-800">
          <div className="flex items-center justify-between text-xs">
            <div className="flex items-center gap-3">
                          ...
                          if (editBranchName.trim() && editBranchName !== gitCommands.currentBranch) {
                            ...
                            try {
                              ...
                              if (response.success) {
                                ...
                                setRenameError(''); // Clear error on success
                              } else {
                                console.error('Failed to rename worktree:', response.error);
                                setRenameError(`Failed to rename worktree: ${response.error}`);
                              }
                            } catch (error) {
                              console.error('Error renaming worktree:', error);
                              setRenameError('Failed to rename worktree');
                            } finally {
                              setIsRenamingBranch(false);
                            }
                          } else {
                            setIsEditingBranch(false);
                            setRenameError(''); // Clear error when canceling
                          }
                        } else if (e.key === 'Escape') {
                          setIsEditingBranch(false);
                          setEditBranchName(gitCommands.currentBranch || '');
                          setRenameError(''); // Clear error when escaping
                        }
                      }}
                      onBlur={() => {
                        // Only cancel edit if no rename is in progress
                        if (!isRenamingBranch) {
                          setIsEditingBranch(false);
                          setEditBranchName(gitCommands.currentBranch || '');
                          setRenameError(''); // Clear error when canceling
                      ...
                      {!activeSession.archived && (
                        <button
                          onClick={() => {
                            ...
                            setRenameError(''); // Clear any previous errors when starting to edit
                          }}
              ...
              {/* Inline error message for worktree renaming */}
              {renameError && (
                <div className="mt-2 px-3 py-2 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-md">
                  <div className="flex items-center gap-2">
                    <svg className="w-4 h-4 text-red-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className="text-sm text-red-700 dark:text-red-300">{renameError}</span>
                    <button
